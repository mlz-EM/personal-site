...
FALLBACK_MIRROR_URL = f"https://r.jina.ai/http://scholar.google.com/citations?user={SCHOLAR_USER_ID}&hl=en"

METRIC_PATTERN = re.compile(r'<td class="gsc_rsb_std">([^<]+)</td>')
PAIR_PATTERN = re.compile(r"(Citations|h-index|i10-index)(.*?)(\d[\d,]*)\s+(\d[\d,]*)", re.IGNORECASE | re.DOTALL)
...
def extract_metrics_from_text(raw_text: str) -> dict[str, int]:
  values = METRIC_PATTERN.findall(raw_text)
  if len(values) >= 6:
    numbers = [parse_int(value) for value in values[:6]]
    return {
      "citationsAll": numbers[0],
  ...
  for match in PAIR_PATTERN.finditer(raw_text):
    ...
    all_value = parse_int(match.group(3))
    recent_value = parse_int(match.group(4))
    extracted[label] = (all_value, recent_value)

  if not all(key in extracted for key in ("citations", "h-index", "i10-index")):
    raise RuntimeError("Could not parse metrics from fallback response.")

  return {
    "citationsAll": extracted["citations"][0],
    "citationsSince2019": extracted["citations"][1],
    "hIndexAll": extracted["h-index"][0],
    "hIndexSince2019": extracted["h-index"][1],
    "i10IndexAll": extracted["i10-index"][0],
    "i10IndexSince2019": extracted["i10-index"][1],
  }
...
def main() -> int:
  ...
  except (HTTPError, URLError, TimeoutError, ValueError, RuntimeError) as primary_exc:
    try:
      mirror_text = fetch_html(FALLBACK_MIRROR_URL)
      metrics = extract_metrics_from_text(mirror_text)
      payload["sourceUrl"] = FALLBACK_MIRROR_URL
      print("Scholar direct fetch failed; fallback mirror parse succeeded.", file=sys.stderr)
    except (HTTPError, URLError, TimeoutError, ValueError, RuntimeError) as fallback_exc:
      payload["status"] = "stale"
      payload["lastAttemptAt"] = attempt_time
      payload["lastError"] = (
        f"direct: {primary_exc}; fallback: {fallback_exc}"
      )
      OUTPUT_PATH.write_text(f"{json.dumps(payload, indent=2)}\n", encoding="utf-8")
      print(
        "Scholar metrics fetch failed; keeping last good values. "
        f"Reason: direct={primary_exc}; fallback={fallback_exc}",
        file=sys.stderr,
      )
      return 1
